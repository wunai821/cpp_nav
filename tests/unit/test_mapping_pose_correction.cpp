...
std::vector<rm_nav::data::PointXYZI> MakeWorldPoints() {
  ...
  for (int index = 0; index < 36; ++index) {
    ...
    point.x = 0.8F + 0.07F * static_cast<float>(index % 6);
    point.y = -0.8F + 0.12F * static_cast<float>(index / 6);
    point.z = 0.6F;
...
rm_nav::data::SyncedFrame MakeFrameFromTruePose(
    const std::vector<rm_nav::data::PointXYZI>& world_points, rm_nav::common::TimePoint stamp,
    std::uint32_t frame_index, float true_x, float true_y, float true_yaw) {
...
int main() {
  ...
  config.loop_correction_min_score = 0.2;
  config.loop_correction_max_translation_m = 0.3;
  config.loop_correction_max_yaw_rad = 0.2;
  config.loop_correction_max_consecutive_failures = 3;
  config.loop_correction_apply_translation_step_m = 0.05;
  config.loop_correction_apply_yaw_step_rad = 0.03;
  ...
  const auto pose0_raw = MakePose(t0, 0.0F, 0.0F, 0.0F);
  assert(engine.Update(MakeFrameFromTruePose(world_points, t0, 1U, 0.0F, 0.0F, 0.0F), pose0_raw)
             .ok());

  const auto pose1_raw = MakePose(t0 + std::chrono::seconds(1), 1.0F, 0.0F, 0.0F);
  assert(engine.Update(MakeFrameFromTruePose(world_points, t0 + std::chrono::seconds(1), 2U, 1.0F,
                                             0.0F, 0.0F),
                       pose1_raw)
             .ok());

  const auto pose2_raw = MakePose(t0 + std::chrono::seconds(7), 0.18F, 0.0F, 0.08F);
  assert(engine.Update(MakeFrameFromTruePose(world_points, t0 + std::chrono::seconds(7), 3U, 0.0F,
                                             0.0F, 0.0F),
                       pose2_raw)
             .ok());
  const auto correction_decision = engine.LatestLoopCorrection();
  assert(correction_decision.accepted);

  const auto pose3_raw = MakePose(t0 + std::chrono::seconds(8), 0.18F, 0.0F, 0.08F);
  assert(engine.Update(MakeFrameFromTruePose(world_points, t0 + std::chrono::seconds(8), 4U, 0.0F,
                                             0.0F, 0.0F),
                       pose3_raw)
             .ok());

  const auto applied = engine.AppliedPoseCorrection();
  const auto corrected_pose = engine.LatestResult().map_to_base;
  assert(applied.is_valid);
  assert(applied.position.x < 0.0F);
  assert(std::fabs(applied.position.x) <= 0.051F);
  assert(std::fabs(applied.rpy.z) <= 0.031F);
  assert(corrected_pose.position.x < pose3_raw.position.x);
  assert(corrected_pose.rpy.z < pose3_raw.rpy.z);

  std::cout << "test_mapping_pose_correction passed\n";
  return 0;
}
